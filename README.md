...
本项目是基于 C++利用面向过程与静态数据结构（结构体数组）实现的多用户密码管理系统（极简KV数据库）。
项目核心目标是：实现多用户的数据隔离、数据的加密持久化（本地文件读写）
## 二、系统五层架构

1. 交互层（UI / 菜单）：main()、adminMenu()、userMenu()。
   · 职责：处理用户输入，提供双菜单角色分流（管理员 / 普通用户），处理输入流异常（防止死循环）。
2. 状态与权限层（Session）：ruletoken、userUid。
   · 职责：记录当前会话的权限和用户身份。ruletoken 决定可见菜单，userUid 用于隔离数据。
3. 业务逻辑层（数据定位与操作）：Insert / Delete (模板)、Locate* 系列函数、LocateMyDataBySeq。
   · 职责：实现核心的增删改查，以及“用户视角序号”到“数组物理下标”的映射（通过 UID 过滤）。
4. 安全与加密层：encryptDecrypt()、toHex()、fromHex()。
   · 职责：异或加密（密钥 0x7F）保护密码；通过十六进制转码防止控制字符破坏文件解析结构。
5. 持久化层（文件 I/O）：inFile()、outFile()。
   · 职责：启动时全量加载到内存，退出时全量覆盖写回硬盘，实现数据的“可带走”。

## 三、数据流

· 启动阶段：main() 调用 inFile() → 从 users.txt 和 data.txt 读入数据 → 若 userCount == 0 则初始化管理员。
· 操作阶段：登录成功 → 写入 userUid 和 ruletoken → 增删改查（操作内存数组）→ 数据隔离通过 UID 匹配。
· 退出阶段：main() 调用 outFile() → 遍历数组 → 加密并转十六进制 → 全量覆盖写入文件。
